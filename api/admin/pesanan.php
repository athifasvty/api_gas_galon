...
if ($method === 'GET') {
    if (isset($_GET['id'])) {
        // Detail pesanan tertentu dengan item
        $stmt = $conn->prepare("
            SELECT p.*, 
                   ...
                   pm.metode as metode_bayar, pm.status as status_bayar, pm.tanggal_bayar
            FROM pesanan p
        ...
        // Semua pesanan dengan filter opsional
        $where = [];
        $params = [];
...
if ($method === 'DELETE') {
    ...
    // Cek status pesanan
    $stmt = $conn->prepare("SELECT status FROM pesanan WHERE id = ?");
    $stmt->execute([$data->id]);
    $pesanan = $stmt->fetch(PDO::FETCH_ASSOC);
